...
export default function HomePage() {
  ...
  const continueWatching = Object.entries(positions)
    ...
    .map(([streamId, pos]) => {
      ...
      return { streamId, progress: pos.position / pos.duration, name: r.name, logo: r.logo, streamType: r.streamType || "live" };
    }).filter(Boolean) as { streamId: string; progress: number; name: string; logo?: string; streamType: string }[];

  const heroItems = recentItems.slice(0, 6).map((r) => ({ id: r.id, name: r.name, logo: r.logo, streamType: r.streamType }));
  const recentRow = recentItems.slice(0, 30).map((r) => ({ id: r.id, name: r.name, logo: r.logo, streamType: r.streamType }));
  const favRow = favorites.slice(0, 20).map((f) => ({ id: f.id, name: f.name, logo: f.logo, streamType: f.streamType }));
