...
export default function BrowseCategoryPage() {
  ...
  const type = (rawParams?.type ?? "movie") as ContentType;
  const categoryId = (rawParams?.categoryId ?? "") as string;
  ...
  const handlePlay = (item: Channel | Movie | Series) => {
    ...
    nav(`/player/${itemId}?type=${type}&url=${encodeURIComponent(url)}&name=${encodeURIComponent((item as { name?: string }).name ?? "")}&referrer=${encodeURIComponent(referrer)}`);
  };

  if (!isXtream || !creds) {
    return (
      <div className="flex h-full items-center justify-center p-4">
        <ErrorDisplay message="Nicht angemeldet" onRetry={() => { nav("/login"); }} />
      </div>
    );
  }
